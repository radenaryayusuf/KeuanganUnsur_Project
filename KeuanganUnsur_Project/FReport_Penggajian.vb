Imports System.IO
Public Class FReport_Penggajian
    Public a As String
    ...
    Private Sub FReport_Penggajian_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call koneksi()
    ...
    Private Sub cb_penerima_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cb_penerima.SelectedIndexChanged
        If DateTimePicker1.Text <> "" And cb_penerima.Text <> "" Then
            dgv.DataSource = getsql("select * from q_penggajian where tgl = '" & DateTimePicker1.Text & "' and nama_pegawai = '" & cb_penerima.Text & "'")
            Dim jumlahuang As Double
            jumlahuang = 0
            For t As Integer = 0 To dgv.Rows.Count - 1
                jumlahuang = jumlahuang + Val(dgv.Rows(t).Cells(6).Value)
            Next
    ...
    Private Sub DateTimePicker1_ValueChanged(sender As Object, e As EventArgs) Handles DateTimePicker1.ValueChanged
        dgv.DataSource = getsql("select * from q_penggajian where tgl = '" & DateTimePicker1.Text & "'")
        cb_penerima.Text = ""
        cb_penerima.Items.Clear()

        For x As Integer = 0 To dgv.Rows.Count - 1
            cb_penerima.Items.Add(dgv.Rows(x).Cells(2).Value)
    ...
    Private Sub CetakKondisi()
        ...
        dt = New DataTable
        dt = getsql("select * from q_penggajian where nama_pegawai = '" & cb_penerima.Text & "'")

        For s = 0 To (dt.Rows.Count - 1)
            rpt = File.ReadAllText(Path.GetDirectoryName(Application.ExecutablePath) & "\kwitansigaji.mht")
        ...
        For i = 0 To (dt.Rows.Count - 1)
            cPart = File.ReadAllText(Path.GetDirectoryName(Application.ExecutablePath) & "\kwitansigaji_content.mht")
        ...
        For l = 0 To (dt.Rows.Count - 1)
            ...
            rpt = Replace(rpt, "[pn]", dt.Rows(0)("nama_pegawai").ToString)
            rpt = Replace(rpt, "[jml]", total)
        Next
        File.WriteAllText("E:\Penggajian" & sekarang & ".xls", rpt)
        ShellExecuteA(Me.Handle, "open", "E:\Penggajian" & sekarang & ".xls", "", "", 1)
    End Sub
